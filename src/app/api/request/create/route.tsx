...
export async function POST(request: NextRequest) {
    ...
    try {
        const { type, method, URL, headers, body, email, collectionName } = await request.json();
        if (!type || !method || !URL || !headers || !body || !email || !collectionName) {
        ...
        await User.findOneAndUpdate({ email: email, "collections.name": collectionName }, { $push: { "collections.$.requests": { type: type, method: method, URL: URL, headers: headers, body: body } } }, { new: true });
